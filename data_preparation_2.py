...
def process_image(file_path, base_dir, label_folders, coords):
    ...
    cropped_image = cv2.resize(cropped_image, (96, 96), interpolation=cv2.INTER_AREA)
    ...
    truth_binary = cv2.resize(truth_binary, (96, 96), interpolation=cv2.INTER_NEAREST).astype(np.uint8)
    truth_parts = cv2.resize(truth_parts, (96, 96), interpolation=cv2.INTER_NEAREST).astype(np.uint8)
    truth_instruments = cv2.resize(truth_instruments, (96, 96), interpolation=cv2.INTER_NEAREST).astype(np.uint8)
