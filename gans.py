...
class robotic_instrument(Dataset):
    """
...
if __name__ == '__main__':
    ...
    BATCH_SIZE = 15
    NUM_CHANNELS = 3  # Number of channels in the training images. Using 3d volume -> Channels = z_dim = 48 
    ...
    folder = './data/cropped_images/instrument_dataset_1/training_image_data'
    instrument_data = robotic_instrument(folder, resize_size=64)
    instrument_data_loader = DataLoader(instrument_data, 
                                 batch_size = BATCH_SIZE,
                                 shuffle = True)
    
    BASE_DIR = os.path.join(os.getcwd(), 'GANS_results') # for saving results into 
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, data in enumerate(instrument_data_loader):
            ...
            if i % 50 == 0:
                print('[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f\tD(x): %.4f\tD(G(z)): %.4f / %.4f'
                    % (epoch, NUM_EPOCHS, i, len(instrument_data_loader),
            ...
            if (iters % UPDATE_FREQ == 0) or ((epoch == NUM_EPOCHS -1) and (i == len(instrument_data_loader)-1)):
                with torch.no_grad():
