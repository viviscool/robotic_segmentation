...
def val_bin(model, loss_function, valid_loader, num_classes=None):
    with torch.no_grad():
        ...
        for images, masks in valid_loader:
            ...
            intersection = (images * masks).sum(dim=(-2, -1))

            union = masks.sum(dim=(-2, -1)) + images.sum(dim=(-2, -1))

            iou_calc = ((intersection + epsilon) / (union - intersection + epsilon)).detach().numpy().tolist()
...
class robotic_data_segmentation(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = {"image": training_image, "mask": mask}
...
def main():
    ...
    model_infor_directory_path = current_working_directory/ 'model_info'
    ...
    validation_transform = Compose([PadIfNeeded(min_height=1024, min_width=1280, p=1), Normalize(p=1)])

    train_transform = Compose([PadIfNeeded(min_height=1024, min_width=1280, p=1), Normalize(p=1), HorizontalFlip(p =0.6), 
                               VerticalFlip(p=0.6), RandomBrightnessContrast(p=0.4), GaussNoise(p= 0.3),OpticalDistortion(p=0.3),
                               MotionBlur(p=0.3), HueSaturationValue(p=0.3)])

    training_dataset = robotic_data_segmentation(training_images, transform=train_transform, problem_type=problem_type)

    

    training_data_loader = DataLoader(training_dataset, batch_size=batch_size, shuffle=True, num_workers=12, pin_memory= torch.cuda.is_available())


    validation_dataset = robotic_data_segmentation(validation_images, transform=validation_transform,  problem_type=problem_type)
  

    validation_data_loader = DataLoader(validation_dataset, batch_size=batch_size, shuffle=False, num_workers=12,pin_memory= torch.cuda.is_available()) 
    print(len(training_dataset))
    print(len(training_data_loader))
    
    optimizer = Adam(model.parameters(), lr = learning_rate)

    model_directory = model_infor_directory_path/f'model_{str(fold)}'
